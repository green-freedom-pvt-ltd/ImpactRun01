...
@property (nonatomic, strong) RCTRefreshControl *_refreshControl;
...
- (void)dockClosestSectionHeader
{
  ...
  if (self._refreshControl != nil && self._refreshControl.refreshing) {
    scrollTop -= self._refreshControl.frame.size.height;
  }
...
- (void)setRefreshControl:(RCTRefreshControl *)_refreshControl
{
  if (_refreshControl) {
    [_refreshControl removeFromSuperview];
  }
  _refreshControl = _refreshControl;
...
- (void)insertReactSubview:(UIView *)view atIndex:(__unused NSInteger)atIndex
{
  if ([view isKindOfClass:[RCTRefreshControl class]]) {
    _scrollView._refreshControl = (RCTRefreshControl*)view;
  } else {
...
- (void)removeReactSubview:(UIView *)subview
{
  if ([subview isKindOfClass:[RCTRefreshControl class]]) {
    _scrollView._refreshControl = nil;
  } else {
...
{
  if (_contentView && _scrollView._refreshControl) {
    return @[_contentView, _scrollView._refreshControl];
  }
...
- (void)layoutSubviews
{
  ...
  RCTRefreshControl *_refreshControl = _scrollView._refreshControl;
  if (_refreshControl && _refreshControl.refreshing) {
    _refreshControl.frame = (CGRect){_scrollView.contentOffset, {_scrollView.frame.size.width, _refreshControl.frame.size.height}};
  }
...
- (void)setOnRefreshStart:(RCTDirectEventBlock)onRefreshStart
{
  ...
  if (!_scrollView._refreshControl) {
    RCTRefreshControl *_refreshControl = [RCTRefreshControl new];
    [_refreshControl addTarget:self action:@selector(refreshControlValueChanged) forControlEvents:UIControlEventValueChanged];
    _scrollView._refreshControl = _refreshControl;
  }
}
...
- (void)endRefreshing
{
  [_scrollView._refreshControl endRefreshing];
}
