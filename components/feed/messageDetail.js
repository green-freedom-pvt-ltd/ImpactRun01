...
class CauseDetail extends Component {
     ...
     state = {
        ...
        error: null,
        res: null,
        path:"https://i.imgur.com/5EOyTDQ.jpg",
        visible:false,
        value: {
          format: "png",
          quality: 0.9,
     ...
     onPresstakescreenShort(home){
      ...
      takeSnapshot(this.refs[home], this.state.value)
      .then(res =>
        ...
        Image.getSize(
          res,
          (width, height) => (ActionSheetIOS.showShareActionSheetWithOptions({
          url:"AppIcon.png",
          message: 'message to go with the shared url',
          subject: 'a subject to go in the email heading',
          excludedActivityTypes: [
      ...
      .then((res)=>{
        ...
        console.log("data:image/"+this.state.value.format+";base64,"+res);
        this.setState({
          error: null,
          res,
      ...
      subject: "Share Link" //  for email
       ...
       let shareOptions = {
        ...
        return (
          <ScrollView  style={{position:'absolute',height:deviceHeight,width:deviceWidth,backgroundColor: '#fff'}}>  
            ...
            <View>
            <Image  ref = "home" style={{width:deviceWidth, height:deviceHeight/2}} source={{uri:data.message_image}}></Image>
            <Text>{data.message_title}</Text>
            <Text>{data.message_description}</Text>
            </View>
            <TouchableOpacity onPress={()=> this.onPresstakescreenShort("home")}><Text>Take your screen short</Text></TouchableOpacity>
             <Image style={{width:deviceWidth, height:deviceHeight/2}} source={this.state.previewSource}></Image>
             <TouchableOpacity
         ...
         <Text onPress={this.showShareActionSheet} >
          Click to show the Share ActionSheet
        </Text>
             <TouchableOpacity onPress={()=> this.onOpen()}><Text>share</Text></TouchableOpacity>
