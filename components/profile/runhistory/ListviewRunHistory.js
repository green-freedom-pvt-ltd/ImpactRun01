...
var RunHostoryListView = React.createClass({
  ...
  emptyView(refreshCallback) {
    ...
    return (
      <View style={[this.defaultStyles.defaultView, this.props.customStyles.defaultView]}>
        ...
        <TouchableHighlight
          style={{borderRadius:25,justifyContent: 'center',alignItems: 'center', height:50,width:50}}
          underlayColor='#c8c7cc'
  ...
  getInitialState() {
    ...
    if (this.props.withSections === true) {
      ds = new ListView.DataSource({
        rowHasChanged: (row1, row2) => row1.run_id !== row2.run_id,
        sectionHeaderHasChanged: (section1, section2) => section1.run_id !== section2.run_id,
      });
      return {
  ...
  convertFoodArrayToMap(rows) {
      ...
      var RunDate = foodItem.start_time
      ...
      if (!foodCategoryMap[day]) {
        // Create an entry in the map for the category if it hasn't yet been created
        foodCategoryMap[day] = [];
      }
      foodCategoryMap[day].push(foodItem);      
      });
...
module.exports = RunHostoryListView;
